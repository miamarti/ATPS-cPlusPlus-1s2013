#ifndef CLIENTE_H
#define CLIENTE_H

#include <iostream>
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include "baseDAO.h"
#define clear "\e[H\e[2J"
using namespace std;

class Cliente
{
    int intTemp;
    string charTemp;
    BaseDAO dao;
    people cliente;

    public:

    //Telas
    void getTAll(){
        cout << clear << endl;
        cout.width(10);
        cout << right << "CLIENTES" << endl;
        cout << "____________________________________________________________________________" << endl;
        dao.getClientes();
        cout << "____________________________________________________________________________" << endl;
        cout << "\nInforme o ID para selecionar ou [x] para ir ao Menu" << endl;
        cout << ": ";
        cin >> charTemp;
        if(charTemp != "x"){
            Cliente::getTId(charTemp);
        }
    }

    void setTData(){
        cout << clear << endl;
        cout.width(10);
        cout << right << "CADASTO DE CLIENTES" << endl;
        cout << "____________________________________________________________________________" << endl;

        cout << "Nome: ";
        cin.getline(cliente.name,100);
        cin.getline(cliente.name,100);
        cout << "fone: ";
        cin.getline(cliente.fone,100);
        cout << "email: ";
        cin.getline(cliente.email,100);

        dao.setCliente(cliente.name, cliente.fone, cliente.email);
        cout << clear << endl;
        cout << "Seu registro foi inserido com sucesso" << endl;
        cout << "____________________________________________________________________________" << endl;
        cout << "\nVoltar [Enter]";
        cin.get();
    }

    void getTId(string id){
        if(id == ""){
            cout << clear << "Informe o ID";
            cout << "\n__________________________________________________________\n:";
            cin >> charTemp;
            Cliente::getTId(charTemp);
        } else{
            cout << clear << endl;
            cout.width(10);
            cout << right << "CLIENTE SELECIONADO" << endl;
            cout << "____________________________________________________________________________" << endl;
            dao.getCliente(id);
            cout << "____________________________________________________________________________" << endl;
            cout << "\nVoltar [1] | Excluir [2] | Editar [3]" << endl;
            cout << ": ";
            cin >> intTemp;

            switch (intTemp) {
            case 2:
                dao.deleteCliente(id);
                break;
            case 3:
                Cliente::getTPut(id);
                break;
            default:
                break;
            }
        }
    }

    void getTName(){
        cout << clear << "Informe o Nome";
        cout << "\n__________________________________________________________\n:";
        cin >> charTemp;

        cout << clear << endl;
        cout.width(10);
        cout << right << "RESULTADO" << endl;
        cout << "____________________________________________________________________________" << endl;
        dao.getClienteName(charTemp);
        cout << "____________________________________________________________________________" << endl;
        cout << "\nInforme o ID para selecionar ou [x] para ir ao Menu" << endl;
        cout << ": ";
        cin >> charTemp;
        if(charTemp != "x"){
            Cliente::getTId(charTemp);
        }
    }

    void getTPut(string id){
        cout << clear << endl;
        cout << "Qual coluna deseja editar: ";
        dao.getClienteCol();
        cout << "____________________________________________________________________________" << endl;
        cout << ": ";
        cin >> intTemp;
        cout << clear << endl;
        cout << right << "INFORME O NOVO VALOR" << endl;
        cout << "____________________________________________________________________________" << endl;
        cout << ": ";
        cin.getline(cliente.name,100);
        cin.getline(cliente.name,100);
        dao.putCliente(id,intTemp,cliente.name);
    }
};

#endif // CLIENTE_H
